...
async def main() -> None:
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            check(driving["outbound"]["complete"], "outbound driving cost is complete")
            check(isinstance(driving["outbound"]["total_krw"], int), "outbound total is integer KRW")
            return_complete = bool(driving["return"]["complete"])
            round_trip_complete = bool(driving["round_trip"]["complete"])
            if return_complete and round_trip_complete:
                ...
                check(driving["cost_complete"], "complete driving cost has verified outbound and return legs")
            else:
                check(
                    driving["return"]["toll_krw"] is None
                    and driving["round_trip"]["total_krw"] is None,
                    "unavailable return toll remains unknown instead of zero",
                )
            ...
            if return_complete and round_trip_complete:
                ...
                check(
                    "확인 불가" in await page.locator("#cost-return-total").inner_text(),
                    "UI keeps unknown return total visibly unavailable",
                )
